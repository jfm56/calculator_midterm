...
def test_operation_mapping():
    ...
    # ✅ Compare dictionary keys and values separately to avoid ordering issues
    assert operation_mapping.keys() == expected_mapping.keys()  # ✅ Directly compare keys
    assert all(value == expected_mapping[key] for key, value in operation_mapping.items())  # ✅ Use .items()
